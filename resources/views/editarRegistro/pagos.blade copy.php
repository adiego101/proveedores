<fieldset>
    ...
    <div>
        <table class="table table-hover">
            ...
            @if(isset($proveedor) && $proveedor->pagos->isNotEmpty()))
            @foreach($proveedor->pagos as $pagos)
                <tr id="row_pago' + indice +'">
                    <td><input type="text" class="form-control" aria-describedby="basic-addon1" id="nombre_sucursal' + indice +'" readonly value="{{$sucursal->nombre_sucursal}}" />
                        <input type="hidden" class="form-control" aria-describedby="basic-addon1" id="calle_sucursal' + indice +'" name="calles[]" readonly value="{{$sucursal->calle}}">
                        <input type="hidden" class="form-control" aria-describedby="basic-addon1" id="numero_sucursal' + indice +'" name="numeros[]" readonly value="{{$sucursal->numero}}">
                        <input type="hidden" class="form-control" aria-describedby="basic-addon1" id="dpto_sucursal' + indice +'" name="dptos[]" readonly value="{{$sucursal->dpto}}">
                        <button type="button" name="edit" id="'+ indice +'" class="btn btn-warning btn-sm btn_edit_sucursal" title="editar sucursal"><indice class="fas fa-edit"></i></button> <button type="button" name="remove" id="' + indice +'" class="btn btn-danger btn-sm btn_remove_sucursal" title="quitar sucursal"><indice class="fas fa-trash"></i></button>
                    </td>
                </tr>
            @endforeach
        @endif
